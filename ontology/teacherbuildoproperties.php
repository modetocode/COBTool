...
echo $OUTPUT->heading(get_string('choise_of_object_properties_for_ontology', 'ontology'));
echo '</div> <br/>';
?>
<script>
    $(function() {
...
<script  type="text/javascript">
    function ajax()
    {
        ...
        if (document.getElementById("prikaz").value==1)
            xmlhttp.open("GET","teacheroproperty.php?id="+document.getElementById("id").value+"&tip=1&klaster="+document.getElementById("stekk").value,true);
        else
            xmlhttp.open("GET","teacheroproperty.php?id="+document.getElementById("id").value+"&tip=2&klaster="+document.getElementById("stekk").value,true);
        xmlhttp.send(); 
    }
    function ajax_spoj(id)
    {
        ...
        xmlhttp.open("GET","teacherclasteroproperty.php?id="+document.getElementById("id").value+"&propertyid="+document.getElementById("svojs"+id).value,true);
        xmlhttp.send();
    }
    function Prikaz()
    {
    ...
    function parserRedirect()
    {
        ...
        window.location= "oproperties_claster.php?ids="+ids+"&ontid="+ontid;
    }
    ...
    function ajax_write(tip,value)
    {
        ...
        {  //dodavanje na novo svojstvo
            ...
            var id3="range"+str.substr(3);
            var oCheck2=document.svojstva.elements[id3];
            var string2="";
            if (oCheck2!=null)
            {
                if (oCheck2[0])
            ...
            xmlhttp.open("GET","teacherwrite.php?tip=2&id="+propertyid+"&properties="+properties+"&domeni="+string+"&rangovi="+string2+"&ekvivalentni="+string3+"&disjunktni="+string4,true);
        }
        else
            if (tip==2)
        { //brishenje na novo svojstvo
            ...
            xmlhttp.open("GET","teacherdelete.php?tip=2&properties="+properties+"&delexpressions="+delexpressions+"&delexpressions2="+delexpressions2+"&delexpressions3="+delexpressions3,true);
        }
        else
            if (tip==3)
        { //azuriranje na postoecko svojstvo
            ...
            xmlhttp.open("GET","teacherwriteupdate.php?tip=2&accepted="+string+"&refused="+string2+"&accepted2="+string3+"&refused2="+string4+"&accepted3="+string5+"&refused3="+string6,true);
        }
        else
        { //brisenje na azuriranja na celoto svojstvo
            ...
            xmlhttp.open("GET","teacherdeleteupdate.php?tip=2&expressions="+string+"&expressions2="+string2+"&expressions3="+string3,true);
        }
        xmlhttp.send();
        Prikaz();
    ...
    function KlasteriraniOSvojstva(){
        document.getElementById("heading").style.visibility="Hidden";
        document.getElementById("prikaz").style.visibility="Hidden";
        if (window.XMLHttpRequest)
        {// code for IE7+, Firefox, Chrome, Opera, Safari
        ...
        var ids=document.getElementById("stekk").value;
        var ontid=document.getElementById("id").value;
        xmlhttp.open("GET","oproperties_claster.php?ids="+ids+"&ontid="+ontid,true);
        xmlhttp.send();
    }
    function setStyle()
    {
    ...
    function writePropertyToDB(){//==============================================mozebi ke treba se smeni mesto
        if (window.XMLHttpRequest)
        {// code for IE7+, Firefox, Chrome, Opera, Safari
            xmlhttp=new XMLHttpRequest();
        }
        ...
        var rank_i_domen= new String();
        var rank_i_domen_ida=getSelectedCheckboxValue(document.getElementsByName("exp_cbox"));
        for(var i=0;i<rank_i_domen_ida.length;i++){
            if(i!=0)
                rank_i_domen=rank_i_domen.concat(",");
            rank_i_domen=rank_i_domen.concat(rank_i_domen_ida[i]);
        }
        ...
        if(document.getElementById("funkc").checked){
            a+=1;}
        if(document.getElementById("inver").checked){
            a+=10;} 
        if(document.getElementById("tranz").checked){
            a+=100;} 
        if(document.getElementById("sim").checked){
            a+=1000;} 
        if(document.getElementById("asim").checked){
            a+=10000;} 
        if(document.getElementById("ref").checked){
            a+=100000;} 
        if(document.getElementById("iref").checked){
            a+=1000000;}        

    
        xmlhttp.open("GET","oproperties_claster_write.php?uid="+document.getElementById("userid").value+"&mid="+document.getElementById("moduleid").value+"&ime="+encodeURIComponent(document.getElementById("imeT").value)+"&opis="+encodeURIComponent(document.getElementById("opisT").value)+"&site_ida="+document.getElementById("site_ida").value+"&disjointID="+disj+"&equivalentID="+equiv+"&expressionID="+rank_i_domen+"&attributes="+a+"&inverseID="+document.getElementById("inv").value+"&superpropertyID="+document.getElementById("superproperty").value,true);
        xmlhttp.send();
        refresh();
    }
    ...
    function smeni_vrednosti(){
        ...
        document.getElementById("inver").checked=(novi.charAt(1)=='1');
        document.getElementById("tranz").checked=(novi.charAt(2)=='1');
        document.getElementById("sim").checked=(novi.charAt(3)=='1');
        document.getElementById("asim").checked=(novi.charAt(4)=='1');
        document.getElementById("ref").checked=(novi.charAt(5)=='1');
        document.getElementById("iref").checked=(novi.charAt(6)=='1');
