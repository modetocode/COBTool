...
$domain = $DB->get_records("ontology_property_expression", array("ontology_propertyid" => $id, "type" => "3", "status" => "2"));
$domain2 = $DB->get_records("ontology_property_expression", array("ontology_propertyid" => $id, "type" => "3", "status" => "1", "userid" => $USER->id));
$equproperties = $DB->get_records("ontology_property_equivalent", array("ontology_propertyid" => $id, "type" => "2", "status" => "2"));
$equproperties2 = $DB->get_records("ontology_property_equivalent", array("ontology_propertyid" => $id, "type" => "2", "status" => "1", "userid" => $USER->id));
$disproperties = $DB->get_records("ontology_property_disjoint", array("ontology_propertyid" => $id, "type" => "2", "status" => "2"));
$disproperties2 = $DB->get_records("ontology_property_disjoint", array("ontology_propertyid" => $id, "type" => "2", "status" => "1", "userid" => $USER->id));
?>
...
<div id="Region">
    ...
    if ($property->status == 1) {
        if ($tip == 5 && $red == $property->id) {
            ...
            $DB->delete_records("ontology_property_data", array("id" => $property->id));
        } else {
            ?>
            <b> <?php echo get_string('Dproperty_name', 'ontology') . ':'; ?> </b> <span style="color: #006FFF;"><?php echo $property->name; ?> </span>
            <?php
            if (count($DB->get_records("ontology_property_data", array("superproperty" => $property->id))) == 0)
            ...
            ?>  
            <br />
            <b> <?php echo get_string('Dproperty_description', 'ontology') . ':'; ?> </b>  <?php if ($property->description == "") echo get_string('No_description', 'ontology'); else echo $property->description; ?> <br />
        ...
        ?>
        <b> <?php echo get_string('Dproperty_name', 'ontology') . ':'; ?> </b> <span style="color: #006FFF;"><?php echo $property->name; ?> </span> <br />
        <b> <?php echo get_string('Dproperty_description', 'ontology') . ':'; ?> </b>  <?php if ($property->description == "") echo get_string('No_description', 'ontology'); else echo $property->description; ?> <br />
        <b> <?php echo get_string('Characteristics', 'ontology') . ':'; ?> </b> 
        <?php
        for ($i = 0; $i < strlen($property->attributes); $i++)
    ...
    if ($property->name != 'Основно') {
        ?>
        <br />
        <input type="button" id="dodavanje1" value="<?php echo get_string('Add_new_domain', 'ontology') ?>" onclick="execute_ajax1();"/>
    ...
    <b>  <?php echo get_string('Property_range', 'ontology') . ':'; ?> </b>
    <?php
    if ($property->rang == "") {
    ...
    if (count($equproperties) == 0 && count($equproperties2) == 0) {
        ...
        foreach ($equproperties as $key => $value) {
            if ($tip == '2' && $value->id == $red) {
                ...
                echo get_name_of_dproperty($value->ontology_propertyid2);
                if ($isTeacher)
    ...
    if ($property->name != 'Основно') {
        ?>
                    ...
                    <?php echo data_property_hierarhy('listaequsvojstvo', 'equsvojstvo', 0, $moduleid, $USER->id, 1);
                    ?>
                    <br />
            ...
            <td valign="top">
                ...
                if ($property->name != 'Основно') {
                    ...
                    <?php echo data_property_hierarhy('listadissvojstvo', 'dissvojstvo', 0, $moduleid, $USER->id, 1);
                    ?>
                    <br />
