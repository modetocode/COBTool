...
global $DB;
$properties = $DB->get_records('ontology_property_data', array('status' => '3', 'course_modulesid' => $id));
...
<?php echo get_string('new_data_properties_in_ontology', 'ontology'); ?>
                            </center></span>
                    </div>
...
if (count($podredeni_svojstva) == 0) {
    echo '<tr> <td colspan=3> ' . get_string('no_new_data_properties_for_ontology', 'ontology') . ' </td> </tr>';
}
for ($i = 0; $i < count($podredeni_svojstva); $i++) {
    ?>
                                <tr style="border:1px solid #7CAFFC; color: blue; background: #E8E8E8;">
                                    <td colspan="2">
    <?php echo get_string('property_name', 'ontology') . ': <b> ' . get_name_of_dproperty($podredeni_svojstva[$i]->id[0]) . '</b> <b> ' . get_string('superproperty', 'ontology') . ':' . get_name_of_dproperty($podredeni_svojstva[$i]->superproperty) . ' </b> ' . get_string('rating', 'ontology') . ' ' . $podredeni_svojstva[$i]->rank; ?>  
    ...
    ?>
                                ...
                                <tr>
                                    <td width="20"> </td>
                                    <td> <b>  <?php echo get_string('choose_description_and_characteristicks', 'ontology'); ?> </b> </td>
                                </tr>
    <?php
    ...
    for ($j = 0; $j < count($podredeni_svojstva[$i]->userid); $j++) {
        ...
        $svojstvo = $DB->get_record('ontology_property_data', array('id' => $podredeni_svojstva[$i]->id[$j]));
        if ($svojstvo->attributes == '1')
            $karakteristiki = get_string('functional', 'ontology');
        else
            $karakteristiki = get_string('It_has_no_characteristics', 'ontology');
        $rang = $svojstvo->rang;
        $n = count($podredeni_opisi);
        $podredeni_opisi[$n]->opis = get_opis_of_dproperty($podredeni_svojstva[$i]->id[$j]) . '<br/> <b> &nbsp &nbsp &nbsp ' . get_string('characteristicks', 'ontology') . ': </b>' . $karakteristiki . '<br/> <b> &nbsp &nbsp &nbsp ' . get_string('range', 'ontology') . ': </b>' . $rang;
    ...
    ?>
                                ...
                                for ($j = 0; $j < count($podredeni_svojstva[$i]->id); $j++) {
                                    $expressions = $DB->get_records('ontology_property_expression', array('status' => '3', 'type' => '3', 'ontology_propertyid' => $podredeni_svojstva[$i]->id[$j]));
                                ...
                                $allequivalent = array();
                                for ($j = 0; $j < count($podredeni_svojstva[$i]->id); $j++) {
                                    $expressions = $DB->get_records('ontology_property_equivalent', array('status' => '3', 'type' => '2', 'ontology_propertyid' => $podredeni_svojstva[$i]->id[$j]));
                                    foreach ($expressions as $key => $value) {
                                        $allequivalent[] = $value;
                                    }
                                }
                                ...
                                for ($j = 0; $j < count($allequivalent); $j++) {
                                    ...
                                    for ($k = 0; $k < count($podredeni_ekvivalentni); $k++) {
                                        if ($podredeni_ekvivalentni[$k]->text == get_name_of_dproperty($allequivalent[$j]->ontology_propertyid2)) {
                                    ...
                                    if (!$goima) {
                                        ...
                                        $podredeni_ekvivalentni[$n]->text = get_name_of_dproperty($allequivalent[$j]->ontology_propertyid2);
                                ...
                                if (count($allequivalent) == 0) {
                                    ...
                                    for ($j = 0; $j < count($podredeni_ekvivalentni); $j++) {
                                        ...
                                        echo get_name_of_dproperty($podredeni_ekvivalentni[$j]->textid);
                                        echo '</input> </td> <td>';
                                ...
                                $alldisjoint = array();
                                for ($j = 0; $j < count($podredeni_svojstva[$i]->id); $j++) {
                                    $expressions = $DB->get_records('ontology_property_disjoint', array('status' => '3', 'type' => '2', 'ontology_propertyid' => $podredeni_svojstva[$i]->id[$j]));
                                    foreach ($expressions as $key => $value) {
                                        $alldisjoint[] = $value;
                                    }
                                }
                                ...
                                for ($j = 0; $j < count($alldisjoint); $j++) {
                                    ...
                                    for ($k = 0; $k < count($podredeni_disjunktni); $k++) {
                                        if ($podredeni_disjunktni[$k]->text == get_name_of_dproperty($alldisjoint[$j]->ontology_propertyid2)) {
                                    ...
                                    if (!$goima) {
                                        ...
                                        $podredeni_disjunktni[$n]->text = get_name_of_dproperty($alldisjoint[$j]->ontology_propertyid2);
                                ...
                                if (count($alldisjoint) == 0) {
                                    ...
                                    for ($j = 0; $j < count($podredeni_disjunktni); $j++) {
                                        ...
                                        echo get_name_of_dproperty($podredeni_disjunktni[$j]->textid);
                                        echo '</input> </td> <td>';
                                ...
                                foreach ($expressions as $key => $value) {
                                    ...
                                    for ($j = 0; $j < count($izraz); $j++) {
                                        if (strlen($izraz[$j]) > 1) {
                                            if ($izraz[$j][0] == '^' && $izraz[$j][1] == 'p') {//proverka dali e svojstvo
                                ...
                                $superpropertyaccepted = $DB->get_records('ontology_property_data', array('id' => $podredeni_svojstva[$i]->superproperty, 'status' => '2'));
                                if (count($superpropertyaccepted) == 0)
                                    echo '<input type="button" id="add' . $i . '" value="' . get_string('add_to_ontology', 'ontology') . '" onClick="insertProperty(this);" disabled="true" style="width:500px;"/><br/>';
                                else
                    ...
                    <div class="ui-dialog-content ui-widget-content">  
                        <table>
                            ...
                            if ($klastertxt == "")
                                ...
                                $prethodna = "";
                                for ($i = 0; $i < count($klaster); $i++) {
                                    if (get_name_of_dproperty($klaster[$i]) != $prethodna)
                                        echo get_name_of_dproperty($klaster[$i]) . '<br/>';
                                    $prethodna = get_name_of_dproperty($klaster[$i]);
                                }
                                echo '<input type="button" value="' . get_string('edit_the_list', 'ontology') . '" onClick="KlasteriraniPSvojstva();"> </input>';
                            }
                            ?>
                                </td>
                            </tr>
                        ...
                        <table align="center">
                            ...
                            foreach ($properties as $key => $value) {
                                ...
                                if (count($expressions) > 0 || count($equ_properties) > 0 || count($dis_properties) > 0) {
                                    ...
                                    echo get_string('property_name', 'ontology') . ': <b> ' . get_name_of_dproperty($key) . '</b> ' . get_string('superproperty', 'ontology') . ': <b>' . get_name_of_dproperty($value->superproperty) . '</b> </td> <td> </td> </tr>';
                                    ...
                                    if (count($equ_properties) == 0)
                                        ...
                                        foreach ($equ_properties as $key2 => $value2) {
                                            ...
                                            echo get_name_of_dproperty($value2->ontology_propertyid2);
                                            echo '</input> </td> <td>';
                                            //echo get_user_profile($podredeni_izrazi[$j]->userid[$k]).' ';
                                            $user = $DB->get_record('user', array('id' => $value2->userid), '*', MUST_EXIST);
                                            echo ' ' . $OUTPUT->user_picture($user, array('size' => 20));
