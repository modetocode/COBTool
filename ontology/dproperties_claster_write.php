...
//echo 'ime:'.$ime.' opis:'.$opis.' nads:'.$nadsvojstvoID.' atrb:'.$attributes.' disj:'.$disjointID.' equ:'.$equivalentID.' expr:'.$expressionID.' rang:'.$rangID.' siteid:'.$site_ida;
...
$property->rang = $rangID;
$property->status = '2';
$property->points = '0';
$propertyid = $DB->insert_record('ontology_property_data', $property);

//zamenuvanje na superklasa na site ostanati podklasi so novata klasa
foreach (explode(" ", $site_ida) as $pid) {
    $raws = $DB->get_records('ontology_property_data', array('superproperty' => $pid));
    foreach ($raws as $raw) {
        $raw->superproperty = $propertyid;
        $DB->update_record('ontology_property_data', $raw);
    }
}
...
foreach (explode(" ", $site_ida) as $pid) {
    $raw = $DB->get_record('ontology_property_data', array('id' => $pid));
    $raw->status = '5';
    $DB->update_record('ontology_property_data', $raw);
    $raws = $DB->get_records('ontology_property_disjoint', array('ontology_propertyid' => $pid, 'type' => 2));
    foreach ($raws as $tmp) {
        $tmp->status = '5';
//        $tmp->ontology_propertyid=$propertyid;
        $DB->update_record('ontology_property_disjoint', $tmp);
    }
    $raws = $DB->get_records('ontology_property_equivalent', array('ontology_propertyid' => $pid, 'type' => 2));
    foreach ($raws as $tmp) {
        $tmp->status = '5';
//        $tmp->ontology_propertyid=$propertyid;
        $DB->update_record('ontology_property_equivalent', $tmp);
    }
    $raws = $DB->get_records('ontology_property_expression', array('ontology_propertyid' => $pid, 'type' => 3));
    foreach ($raws as $tmp) {
        $tmp->status = '5';
//        $tmp->ontology_propertyid=$propertyid;
        $DB->update_record('ontology_property_expression', $tmp);
    }
    smeni_pod_svojstvo($pid, $propertyid);
}
